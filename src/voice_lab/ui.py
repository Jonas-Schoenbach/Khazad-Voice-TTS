...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Khazad Configuration", theme=gr.themes.Soft(primary_hue="orange")) as demo:
        ...
        with tabs:
            ...
            with gr.Tab("⚙️ Configuration"):
                ...
                with gr.Row():
                    ...
                    with gr.Column(variant="panel"):
                        ...
                        vol_slider = gr.Slider(0.1, 1.0, value=defaults.get("volume", 0.4), step=0.1,
                                               label="Kokoro Volume", info="Overall loudness")
                        ...
                        speed_slider_conf = gr.Slider(0.5, 2.0, value=defaults.get("speed", 1.1), step=0.1,
                                                      label="TTS Speed", info="Higher = Faster")

                    # --- GPU MODE ---
                    with gr.Column(variant="panel"):
                        gr.Markdown("## GPU Mode (LuxTTS)")
                        lux_vol_slider = gr.Slider(0.1, 1.0, value=defaults.get("lux_volume", 0.4), step=0.1,
                                                   label="Lux Volume")
                        ...
                        gr.Markdown("*Note: TTS Speed setting (left) applies to LuxTTS as well.*")

                        steps_slider_conf = gr.Slider(minimum=4, maximum=16, value=defaults.get("steps", 6), step=1,
                        ...
                        chunk_radio = gr.Radio(
                            ...
                            type="index",
                            value="2 (Standard)" if defaults.get("chunk_size", 2) == 2 else "1 (Ultra Fast)",
                            label="Chunk Size",
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        tess_input = gr.Textbox(value=defaults.get("tesseract", ""), label="Tesseract Path",
                                                info="Full path to tesseract.exe", scale=2)
                        thresh_slider = gr.Slider(0.3, 0.7, value=defaults.get("threshold", 0.5), step=0.05,
            ...
            with gr.Tab("🎙️ LuxTTS Tester & Custom Voice Adder", id="test_tab"):
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        with gr.Group(visible=True) as grp_library:
                            lib_category = gr.Dropdown(choices=lib.get_library_categories(), label="Category",
                                                       interactive=True)
                            lib_sample = gr.Dropdown(choices=[], label="Select Sample File", interactive=True)
                            btn_load_lib = gr.Button("Load Voice Data", size="sm")

                        # Upload Group
                        with gr.Group(visible=False) as grp_upload:
                            gr.Markdown("*Upload a clean voice sample (wav/flac) and make sure the transcript matches the audio.*")
                            upload_file_btn = gr.File(label="Upload Audio", file_count="single",
                                                      file_types=[".wav", ".flac", ".mp3"])

                        # Shared Player & Transcript
                        ref_input = gr.Audio(label="Reference Audio Player", type="filepath", interactive=True)
                        transcribe_btn = gr.Button("Auto-Transcribe (Whisper)", size="sm")
                        ref_text_input = gr.Textbox(label="Transcript (auto-transcripts trimmed to 20s)", lines=2,
                                                    placeholder="Text matches the audio...")

                    # RIGHT COLUMN: OUTPUT
                    with gr.Column():
                        target_text_input = gr.Textbox(label="Text to Speak (or type your own)", value=SAMPLE_SCRIPTS[0], lines=2)
                        example_selector = gr.Dropdown(choices=SAMPLE_NAMES, value=SAMPLE_NAMES[0],
                                                       label="Load Sample Text")

                        with gr.Group():
                            gr.Markdown("#### Current Settings (Auto-Applied)")
                            lab_speed = gr.Slider(0.5, 2.0, value=defaults.get("speed", 1.1), label="Test Speed")
                            lab_steps = gr.Slider(4, 16, value=defaults.get("steps", 6), step=1, label="Test Steps")
                ...
                with gr.Row():
                    folder_dropdown = gr.Dropdown(choices=lib.get_voice_folders(), label="Category",
                ...
                def save_wrapper(vol, lux_vol, spd, stp, tess, chk_idx, thresh):
                    ...
                    msg, new_speed, new_steps = cfg.save_settings(vol, lux_vol, spd, stp, thresh, tess, real_chunk)
                    return msg + "\n\n🚀 Configuration Saved! Switch tabs to test.", new_speed, new_steps

                save_btn.click(
                    save_wrapper,
                    inputs=[vol_slider, lux_vol_slider, speed_slider_conf, steps_slider_conf, tess_input, chunk_radio,
                            thresh_slider],
                ...
                mode_radio.change(toggle_mode, inputs=mode_radio, outputs=[grp_library, grp_upload])

                # 3. Library Handling
                def update_samples(cat):
                    return gr.Dropdown(choices=lib.get_samples_for_category(cat), value=None)

                lib_category.change(update_samples, inputs=lib_category, outputs=lib_sample)
                ...
                btn_load_lib.click(load_from_lib, inputs=[lib_category, lib_sample],
                                   outputs=[ref_input, ref_text_input])
                lib_sample.change(load_from_lib, inputs=[lib_category, lib_sample], outputs=[ref_input, ref_text_input])

                # 4. Upload & Processing
                upload_file_btn.change(lambda f: f, inputs=upload_file_btn, outputs=ref_input)
                ref_input.upload(lib.trim_audio, inputs=[ref_input], outputs=[ref_input])

                def run_transcribe(audio):
                    return engine.auto_transcribe(audio, lib.trim_audio)

                transcribe_btn.click(run_transcribe, inputs=[ref_input], outputs=[ref_text_input])

                example_selector.change(lambda x: SCRIPT_MAP[x], inputs=example_selector, outputs=target_text_input)

                # 5. Generation
                def run_gen(text, audio, transcript, spd, steps):
                    try:
                        # audio_data comes as (rate, numpy_array) from engine
                        audio_data = engine.generate_preview(text, audio, transcript, spd, steps, lib.trim_audio)
                        return audio_data, "Generation Successful"
                    except Exception as e:
                        return None, f"Error: {str(e)}"

                test_btn.click(run_gen, inputs=[target_text_input, ref_input, ref_text_input, lab_speed, lab_steps],
                               outputs=[audio_output, status_msg])

                save_voice_btn.click(lib.save_voice, inputs=[ref_input, folder_dropdown, name_input, ref_text_input],
                                     outputs=[save_status])
